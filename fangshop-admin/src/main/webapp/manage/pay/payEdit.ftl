	...
	<table class="table table-bordered" style="width: 100%; margin: auto;">
		<tr style="background-color: #1c94c4">
			<td colspan="2" style="background-color: #1c94c4; text-align: left;">
				<strong style="color: white;">修改支付方式</strong>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2" style="text-align: center;">
				<button class="btn btn-success" id="btnEdit" name="btnEdit">保存</button> 
			</td>
		</tr>
	</table>
</div>
<script type="text/javascript">
	$(function(){
		...
		<#if (pay.seller)??>
	       $('#seller').val('${pay.seller}');
	    </#if>
		<#if (pay.order1)??>
	       $('#order1').val('${pay.order1}');
	    </#if>
		<#if (pay.partner)??>
	...
	function update(){
		...
		$.ajax({
			async : true,
			type : "POST",
			url : "${basepath}/manage/pay/update",
			cache : false,
			timeout : 60 * 60 * 1000,
			dataType : "json",
			data : data,
