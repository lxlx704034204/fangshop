...
<script type="text/javascript">
	...
	function add() {
		window.location = "${basepath}/manage/oss/create";
	};
	function search() {
		...
		$.ajax({
			async : true,
			type : "POST",
			url : "${basepath}/manage/oss/search",
			cache : false,
			timeout : 60 * 60 * 1000,
			dataType : "json",
			data : data,
	...
	function SetTableData(rows) {
		$('#table-ossList').bootstrapTable('destroy').bootstrapTable(
						{
							data : rows,
							height : 650,
							striped : true,
	...
	function del(id){
		if(window.confirm("确定要删除吗？")){
			...
			$.ajax({
				async : true,
				type : "POST",
				url : "${basepath}/manage/oss/delete",
				cache : false,
				timeout : 60 * 60 * 1000,
				dataType : "json",
				data : data,
