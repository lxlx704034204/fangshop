...
var callback;//增加回调，该函数必须明确返回true或false
var cal;
...
var topY = 0, leftX = 0; //自定义定位偏移量 2009-08-19 由 Echina添加 
//选择日期 → 由 Echina 2009-06-10 添加，通过 ID 来选日期 
...
//选择日期 → 由 Echina 2009-06-25 添加 
function SelectDate(obj, strFormat, x, y, cbk, initobj) {
    if (cbk) { callback = cbk; }
    leftX = (x == null) ? leftX : x;
    topY = (y == null) ? topY : y;//自定义定位偏移量 2009-02-11 由 Echina添加 
    if (document.getElementById("ContainerPanel") == null) { InitContainerPanel(obj); }
    var date = new Date();
    var by = date.getFullYear() - 50; //最小值 → 50 年前 
    var ey = date.getFullYear() + 50; //最大值 → 50 年后 
    //cal = new Calendar(by, ey,1,strFormat); //初始化英文版，0 为中文版 
    cal = (cal == null) ? new Calendar(by, ey, 0) : cal; //不用每次都初始化 2009-12-03 修正 
...
2009-06-25 由 Echina 修改为根据用户指定的 style 来确定； 
*/
...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {
    ...
    this.colors = {
        ...
        "cur_bg": "#33aacc", //当日日期单元格背影色 
        "sel_bg": "#d0d0d0", //已被选择的日期单元格背影色 2009-12-03 Echina添加 
    ...
    */
    //this.changeSelect(); 
    //this.bindData(); //2009-12-30 由民工.砖家注释 
}
/**//**//**//**//**//**//**//** 
* 日历类属性（语言包，可自由扩展） 
*/
...
Calendar.prototype.draw = function () {
    calendar = this;
    var mvAry = [];
    //mvAry[mvAry.length] = ' <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2009-12-01 由Echina改用 Div 
    ...
    obj.onclick = function () {
        calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*///2009-09-14 由Echina注释 
    }
    ...
    obj.onclick = function () {
        ...
        if (callback) {
            if (callback(calendar.dateControl, today)) {
...
Calendar.prototype.bindYear = function () {
    var cy = this.calendarYear;//2009-12-01 由Echina修改 
    cy.length = 0;
...
Calendar.prototype.bindMonth = function () {
    var cm = this.calendarMonth;//2009-12-01 由Echina修改 
    cm.length = 0;
    for (var i = 0; i < 12; i++) {
...
Calendar.prototype.update = function (e) {
    this.year = e.calendarYear.options[e.calendarYear.selectedIndex].value;//2009-12-01 由Echina修改 
...
Calendar.prototype.bindData = function () {
    ...
    //2009-12-30 由民工.砖家修改 在Firefox 下年份错误 
    ...
    for (var i = 0; i < tds.length; i++) {
        ...
        if (dateArray[i] != "&nbsp;") {
            ...
            tds[i].onclick = function () {
                ...
                {
                    var _selectDate = new Date(calendar.date.getFullYear(), calendar.date.getMonth(), this.innerHTML);
                    if (callback) {
                        if (callback(calendar.dateControl, _selectDate)) {
                            calendar.ReturnDate(_selectDate.format(calendar.dateFormatStyle));
                        }
                    } else {
                        calendar.ReturnDate(_selectDate.format(calendar.dateFormatStyle));
                    }
            ...
            tds[i].style.cursor = "pointer"; //2009-08-06 由Echina添加，鼠标变成手指状 
...
Calendar.prototype.show = function (dateObj, popControl) {
    ...
    var now = new Date();
    this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle);//2008-01-29 Echina添加 → 若为空则根据dateFormatStyle初始化日期 
    ...
    this.panel.style.left = (xy.x + leftX) + "px"; //由Echina 2009-02-11 修改 → 加入自定义偏移量 
    this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";
    //由Echina 2009-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件 //this.setDisplayStyle("select", "hidden"); 
    ...
    if (!this.dateControl.isTransEvent) {
        ...
        }//2009-09-14 保存主文本框的 onblur ，使其原本的事件不被覆盖 
...
//焦点转移时隐藏日历 → 由Echina 2009-06-25 添加 
Calendar.prototype.onblur = function () {
    if (!isFocus) { this.hide(); }
}
//以下由Echina 2009-07-26 修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方 
function InitContainerPanel(obj) //初始化容器 
{
    //添加当前渲染元素obj，判断obj的position是否固定，确保页面滚动时calendarPanel能够跟随渲染元素
    //赵冬昭 2014-06-25
    var _position = 'absolute';
    if ($(obj).css('position') === 'fixed') { _position = 'fixed'; }
    else {
        $(obj).parents().each(function (index, elem) {
    ...
    var str = '<div id="calendarPanel" style="position: ' + _position + ';display: none;z-index:9999;background-color: #FFFFFF;border: 1px solid #CCCCCC;width:200px;font-size:12px;"></div>';
    if (document.all) {
